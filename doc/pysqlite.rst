...
* APSW is a single file for the extension, :file:`apsw.pyd` on Windows
  ...
  file anywhere your Python session can reach.
...
* APSW has :ref:`execution and row tracers <tracing>`.  sqlite3 has no
  equivalent to :ref:`execution tracers <executiontracer>` and does
  have data adaptors which aren't the same thing as a :ref:`row tracer
  <rowtracer>` (for example you can't skip rows or add a new column to
  each row returned).  sqlite3 does have a `row factory
  <https://docs.python.org/3/library/sqlite3.html?highlight=sqlite#accessing-columns-by-name-instead-of-by-index>`__
...
* sqlite3 has an `adaptor system <https://docs.python.org/3/library/sqlite3.html?highlight=sqlite#converting-sqlite-values-to-custom-python-types>`__`
  ...
  going to recognise your Point class).  You can implement something
  similar in APSW by intercepting :meth:`Cursor.execute` calls that
  suitably mangles the bindings going to SQLite and does something
  similar to the rows the iterator returns.
