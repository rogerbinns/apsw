...
PyInit_apsw(void)
{
  ...
  if (PyType_Ready(&ConnectionType) < 0 || PyType_Ready(&APSWCursorType) < 0 || PyType_Ready(&ZeroBlobBindType) < 0
      ...
      || PyType_Ready(&APSWChangesetIteratorType) <0 || PyType_Ready(&APSWConflictResolutionsType) <0
      || PyType_Ready(&APSWRebaserType) <0
#endif
      )
    goto fail;
...
#define ADD(name, item)                                                                                                \
  ...
  ADD(ConflictResolutions, APSWConflictResolutionsType);
