...
Tokens
  ...
  with values considered to be a sequence of tokens.
...
Ranking
  ...
  to take into account how rare the tokens are (English content has
  "the" all over the place), and other factors based on the function
  like if the tokens are in headings, and how many tokens are in the
