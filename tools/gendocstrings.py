...
def generate_typestubs(items: list[dict]) -> None:
    ...
    with open("src/apswtypes.py", "rt") as f:
        print(f.read().split("# Async Note:")[0], file=out)
        ...
        if klass in {"apsw"}:
            ...
            if item["kind"] == "method":
                ...
                if (
                    ...
                    print(f"{ baseindent }    { decl} { name }{ callable_async(signature) }:", file=async_out)
                ...
                if asyncable and async_category(klass, name, "function") in {"async", "dual", "value"}:
                    print(f"{ baseindent }    { name }: Awaitable[{ attribute_type(item) }]", file=async_out)
...
# Awaitable[X] to return types of Callable.  this is needed to
