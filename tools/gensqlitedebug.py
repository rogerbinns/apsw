...
functions_arg = (
    ...
    ("sqlite3_bind_(double|int64|pointer|zeroblob64)", 3, "sqlite3_db_handle((one))"),
    ("sqlite3_bind_(blob64|text64)", 5, "sqlite3_db_handle((one))"),
    ("sqlite3_clear_bindings", 1, "sqlite3_db_handle((one))"),
    (
        "sqlite3_column_(blob|double|int64|text|bytes|type|count|database_name|table_name|origin_name|decltype|name|value)",
        2,
    ...
    ("sqlite3_result_(blob64|pointer|text64)", 4, "sqlite3_context_db_handle((one))"),
)
    ...
    for pattern, nargs, dbget in functions_arg:
        ...
        if fullmatch(pattern, name):
            if name == "sqlite3_backup_init":
                check = "sqlite3_mutex_held((one)) && sqlite3_mutex_held((three))"
            else:
                check = f"sqlite3_mutex_held(({dbget}))"
            return f"""
    ...
    for pattern, dbget in functions:
        if fullmatch(pattern, name):
            if fullmatch("sqlite3_backup_.*", name):
                check = "sqlite3_mutex_held(self->dest->db) && sqlite3_mutex_held(self->source->db)"
            else:
                check = f"sqlite3_mutex_held({dbget})"
            return f"""
#define {name}(...) ({{
...
# ::TODO:: merge find unwrapped apis into this

# ::TODO:: download a copy
con = apsw.Connection("toc.db")

# ::TODO:: run tools/tocupdate.sql

# ::TODO:: all callers of convert_value_to_pyobject need to assert mutex held
code = ""

for (name,) in con.execute("select name from toc where type='function' and status < 2 order by name"):
    res = code_for_name(name).rstrip().lstrip("\n")
    if res:
        # ::TODO:: backslashify each line
        code += res + "\n\n"
