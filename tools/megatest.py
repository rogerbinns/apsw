...
def dotest(pyver, logdir, pybin, pylib, workdir, sqlitever):
    run("set -e ; cd %s ; ( env LD_LIBRARY_PATH=%s APSW_FORCE_DISTUTILS=t %s setup.py fetch --version=%s --all build_test_extension build_ext --inplace --force --enable-all-extensions test -v ) >%s 2>&1"
        % (workdir, pylib, pybin, sqlitever,
           os.path.abspath(os.path.join(logdir, "buildruntests.txt"))))


def runtest(workdir, pyver, ucs, sqlitever, logdir):
    pybin, pylib = buildpython(
        workdir, pyver, ucs,
        os.path.abspath(os.path.join(logdir, "pybuild.txt")))
...
def main(PYVERS, UCSTEST, SQLITEVERS, concurrency):
    ...
    print "Test starting"
    os.system(
        "rm -rf apsw.so megatestresults 2>/dev/null ; mkdir megatestresults")
    ...
    os.system(
        'rm -rf $HOME/.local/lib/python*/site-packages/apsw* 2>/dev/null')
    print "      done"
        ...
        for ucs in UCSTEST:
            ...
            for sqlitever in SQLITEVERS:
                ...
                workdir = os.path.abspath(
                    os.path.join("work",
                                 "py%s-ucs%d-sq%s" % (pyver, ucs, sqlitever)))
    ...
    print "All builds started, now waiting for them to finish (%d concurrency)" % (
        concurrency, )
    for t in threads:
        t.join()
    print "\nFinished"
    ...
    if pyver > '2.3.0':
        ...
        for v in switchvers:
            if v2i(dirver)[:2] == v2i(v)[:2] and v2i(dirver) >= v2i(v):
                ext = "xz"
                break
        return "https://www.python.org/ftp/python/%s/%sython-%s.tar.%s" % (
            dirver, p, pyver, ext)
    if pyver == '2.3.0':
        pyver = '2.3'
        dirver = '2.3'
    return "https://www.python.org/ftp/python/%s/Python-%s.tgz" % (dirver,
                                                                   pyver)
...
def buildpython(workdir, pyver, ucs, logfilename):
    ...
    if sys.platform.startswith("linux"):
        ...
        if pyver.startswith("2.3") or pyver.startswith("2.4"):
            patch_natty_build(
                os.path.join(workdir, "Python-" + pyver, "setup.py"))
    ...
    run("set -e ; %s cd %s ; cd ?ython-%s ; ./configure %s --enable-unicode=ucs%d --prefix=%s/pyinst  >> %s 2>&1; make >>%s 2>&1; make  %sinstall >>%s 2>&1 ; make clean >/dev/null"
        % (ldflags, workdir, pyver, opt, ucs, workdir, logfilename,
           logfilename, full, logfilename))
    suf = ""
    if pyver >= "3.1":
        suf = "3"
...
SQLITEVERS = ('3.31.0', '3.31.1', '3.32.0', '3.32.1')
...
if __name__ == '__main__':
    ...
    parser.add_option(
        "--ucs",
        dest="ucs",
        help="Unicode character widths to test in bytes [%default]",
        default="2,4")
    parser.add_option(
        "--tasks",
        dest="concurrency",
